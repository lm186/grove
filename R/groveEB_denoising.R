.groveEB.denoising <- function(W, 
  ...
  if (nu != Inf) {
    ...
    empirical_bayes = optim(par=c(log(tau_par),
                                ...
                                log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),log(sigma_par),alpha_par,beta_par,log(nu_par)), fn=fr,  method = method)
  }
  
  else {
    empirical_bayes = optim(par=c(log(tau_par),
                                  log(eta_par[1]), log(eta_par[2]),
                                  log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),log(sigma_par),alpha_par,beta_par), fn=fr,  method = "Nelder-Mead")
  }
...
.groveEB.denoising.fixed.alpha.beta <- function(W, 
  ...
  { 
    ...
    output = fitGroveML(W$D, XX, p = p, 
                      ...
                      gamma_par = c(0.5*exp(gamma.rho)/(1+exp(gamma.rho)),rep(0.5*exp(gamma.kap)/(1+exp(gamma.kap)),p_len-1)),
                      init_state = init_state,
                      nu = exp(nu), sigma = exp(sigma), alpha = alpha, beta = beta)
    if(verbose == TRUE)
      print(-output$marginal_likelihood)
    return(-output$marginal_likelihood)
  }  
  ...
  ans = fitGrove(W$D, XX, p = p, 
                    ...
                    gamma_par = gamma_par, 
                    init_state = init_state,
                    nu = nu_par, sigma = sigma_par, alpha = alpha, beta = beta, n_samp = n_samp)
  ans$data$formula = formula
  ans$data$X = X
  ans$data$W = W
  colnames( ans$data$design_matrix ) = colnames(XX)
