                    ...
                    method ="Nelder-Mead")
{
  ...
  if (nu != Inf) {
    ...
    empirical_bayes = optim(par=c(log(tau_par), ## log(eta_par[1:min(2,p_len)]), ## 
                              ...
                              method = "Nelder-Mead")
{
  ...
  if (nu != Inf) {
    ...
    empirical_bayes = optim(par=c(log(tau_par), 
                                  log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),
                                  log(sigma_par),alpha_par,log(nu_par)), fn=fr,  method = method)
  }
  
  else {
    empirical_bayes = optim(par=c(log(tau_par), 
                                  log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),
                                  log(sigma_par),alpha_par), fn=fr,  method = "Nelder-Mead")
  }
                                        ...
                                        method = "Nelder-Mead")
{
  ...
  if (nu != Inf) {
    ...
    empirical_bayes = optim(par=c(log(tau_par), log(sigma_par),alpha_par,log(nu_par)), fn=fr,  method = method)
  }
  
  else {
    empirical_bayes = optim(par=c(log(tau_par), log(sigma_par),alpha_par), fn=fr,  method = "Nelder-Mead")
  }
...
.groveEB.fixed.alpha <- function(W, 
                                formula, 
                                X, 
                                alpha = 0.5, 
                                beta = 1, 
                                ...
                                method="Nelder-Mead")
{
  ...
  if (nu != Inf) {
    ...
    empirical_bayes = optim(par=c(log(tau_par), ## log(eta_par[1:min(2,p_len)]), ## 
                                  log(eta_par[1]), log(eta_par[2]),
                                  log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),log(sigma_par),log(nu_par)), fn=fr,  method = method)
  }
  
  else {
    empirical_bayes = optim(par=c(log(tau_par), ## log(eta_par[1:min(2,p_len)]), ## 
                                  log(eta_par[1]), log(eta_par[2]),
                                  log(gamma_par/(0.5-gamma_par)), log(gamma_par/(0.5-gamma_par)),log(sigma_par)), fn=fr,  method = "Nelder-Mead")
  }
  ...
  ans = fitGrove(W$D, XX, p = p, 
                    ...
                    gamma_par = gamma_par, 
                    init_state = init_state,
                    nu = nu_par, sigma = sigma_par, alpha = alpha, beta = beta, n_samp = n_samp, transition_mode = transition_mode)
  ans$data$formula = formula
  ans$data$X = X
  ans$data$W = W
  colnames( ans$data$design_matrix ) = colnames(XX)
