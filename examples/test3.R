...
small.signal = rbind(factor.A.1, factor.A.2, factor.B.1, factor.B.2, factor.B.3,factor.C.1,factor.C.2)
...
ans = groveEB(W, frml, XX, alpha = 0.5, beta = 1, nu = 2000, n_samp = 500, verbose = TRUE)
ans$hyper
  
plotStates(ans, block = "Intercept", legend = TRUE)
plotStates(ans, block = "factorA", legend = TRUE)
plotStates(ans, block = "factorB", legend = TRUE)
plotStates(ans, block = "factorC", legend = TRUE)


aa = invDWT( ans, x = c(1,0,0,0,0) )
plotFun(aa)

aa = invDWT( ans, x = c(0,1,0,0,0) )
plotFun(aa)

aa = invDWT( ans, x = c(0,0,1,0,0) )
plotFun(aa)

aa = invDWT( ans, x = c(0,0,0,1,0) )
plotFun(aa)

aa = invDWT( ans, x = c(0,0,0,0,1) )
plotFun(aa)
